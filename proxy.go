...
var (
	RunPort              = 8080                                           // The server port to run on
...
func Proxy(c *gin.Context) {
	...
	destination, err := url.Parse(os.Args[1])
	if err != nil {
		log.Fatal(err)
	}
	...
	//override the host to fixed address
	...
	log.Printf("Forwarding request to %s%s to service %s", c.Request.Host ,c.Request.URL , remote)
